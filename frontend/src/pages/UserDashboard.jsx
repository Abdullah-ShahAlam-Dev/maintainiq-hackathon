...
const AssetsTab = () => {
  ...
  const visibleAssets = useMemo(() => {
    let list = assets.filter(
      (a) =>
        (!categoryFilter ||
          (categoryFilter === OTHER_CATEGORY ? !ASSET_CATEGORIES.includes(a.category) : a.category === categoryFilter)) &&
    ...
    <div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 bg-panel border border-line rounded-sm px-4 py-3 mb-5">
        ...
        <div className="flex gap-2 flex-wrap">
          <select value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)} className="border border-line rounded-sm px-2 py-1.5 text-sm font-sans focus:outline-none focus:border-brand">
            <option value="">All categories</option>
            {ASSET_CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}
            <option value={OTHER_CATEGORY}>Other</option>
          </select>
          <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="border border-line rounded-sm px-2 py-1.5 text-sm font-sans focus:outline-none focus:border-brand">
            <option value="">All statuses</option>
            {statuses.map((s) => <option key={s} value={s}>{s}</option>)}
          </select>
          <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="border border-line rounded-sm px-2 py-1.5 text-sm font-sans focus:outline-none focus:border-brand">
      ...
      {view === 'card' ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {visibleAssets.map((asset) => (
            <div key={asset._id} className="bg-panel border border-line rounded-sm p-4 hover:border-brand transition-colors">
              <h3 className="text-sm font-semibold m-0 mb-1">{asset.name}</h3>
              <p className="text-xs text-muted font-mono m-0">{asset.assetCode} — {asset.location}</p>
              <span className={`inline-block mt-2 mb-2 px-2 py-0.5 rounded-sm border text-[10px] font-mono font-bold uppercase tracking-tag ${STATUS_CLASS[asset.status] || 'text-muted border-muted'}`}>
                {asset.status}
              </span>
          ...
          {visibleAssets.length === 0 && !error && <p className="text-sm text-muted col-span-full">No assets match these filters.</p>}
        </div>
      ) : (
            ...
            <table className="w-full border-collapse">
              ...
              <tbody>
                ...
                {visibleAssets.length === 0 && !error && (
                  <tr><td colSpan={7} className="px-4 py-6 text-sm text-muted text-center">No assets match these filters.</td></tr>
                )}
              </tbody>
            </table>
          </div>
  ...
  return (
    <div>
      ...
      <div className="space-y-3">
          ...
          return (
            <div key={issue._id} className="bg-panel border border-line border-l-4 border-l-brand rounded-sm p-4">
              <div className="flex items-center justify-between">
                ...
                <span className={`px-2 py-0.5 rounded-sm border text-[10px] font-mono font-bold uppercase tracking-tag ${STATUS_STYLES[issue.status] || 'text-[#1d5a8a] border-[#1d5a8a]'}`}>
                  {issue.status}
                </span>
              </div>
              <p className="text-sm text-muted mt-1 mb-0">{issue.description}</p>
              <p className="text-xs font-mono text-muted mt-1 mb-0">
                {issue.assetId ? `Asset: ${issue.assetId.name} (${issue.assetId.assetCode})` : 'Asset: Asset Removed'}
              </p>
              {latest && (
        ...
        {issues.length === 0 && !error && <p className="text-sm text-muted">You haven't reported any issues yet.</p>}
      </div>
    </div>
  );
};
            ...
            return (
              <button
                ...
                className={`group relative whitespace-nowrap font-mono text-xs uppercase tracking-tag px-5 py-3 rounded-t-sm border border-b-0 transition-colors ${
                  isActive ? 'bg-base text-ink border-line' : 'bg-transparent text-white/60 border-transparent hover:text-white'
                }`}
              >
                {isActive && <span className="absolute top-2 left-1/2 -translate-x-1/2 w-1.5 h-1.5 rounded-full bg-hazard" />}
                {!isActive && <span className="absolute top-2 left-1/2 -translate-x-1/2 w-1.5 h-1.5 rounded-full bg-brand opacity-0 group-hover:opacity-100 transition-opacity" />}
