...
const register = async (req, res) => {
  try {
    ...
    if (finalRole === 'technician' && !specialty) {
      return res.status(400).json({ message: 'Specialty is required for technician signup' });
    }
    if (finalRole === 'technician' && !req.file) {
      return res.status(400).json({ message: 'Certification/evidence document is required for technician signup' });
    }
    ...
    res.status(200).json({ message: 'OTP sent to your email. Verify to complete signup.', email: cleanEmail });
  } catch (err) {
...
const verifySignupOtp = async (req, res) => {
  try {
    ...
    if (Date.now() > user.otpExpiry) return res.status(400).json({ message: 'OTP expired — please request a new one' });
    ...
    res.json({ message: 'Email verified. Your account is awaiting admin approval.', pendingApproval: true });
  } catch (err) {
    res.status(500).json({ message: 'Server error', error: err.message });
  }
};
...
const login = async (req, res) => {
  try {
    ...
    if (user.status === 'revoked') {
      return res.status(403).json({ message: 'Your access has been revoked. Contact an administrator.' });
    }
...
const getPendingUsers = async (req, res) => {
  try {
    ...
    if (!role || !['admin', 'technician'].includes(role)) {
      return res.status(400).json({ message: 'role query param must be "admin" or "technician"' });
    }
...
const getManagementList = async (req, res) => {
  try {
    ...
    if (!role || !['admin', 'technician', 'user'].includes(role)) {
      return res.status(400).json({ message: 'role query param must be "admin", "technician" or "user"' });
    }
    if (role === 'admin' && req.user.role !== 'superadmin') {
      return res.status(403).json({ message: 'Access denied' });
    }

    const users = await User.find({ role, status: { $in: ['approved', 'revoked'] } })
...
const deleteUser = async (req, res) => {
  try {
    ...
    if (target.status !== 'revoked') {
      return res.status(400).json({ message: 'Only a revoked account can be deleted' });
    }
    if (target.role === 'admin' && req.user.role !== 'superadmin') {
      return res.status(403).json({ message: 'Only a Super Admin can delete an admin account' });
    }
...
const getTechnicians = async (req, res) => {
  try {
    const technicians = await User.find({ role: 'technician', status: 'approved' }).select('name email');
